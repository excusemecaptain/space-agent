...
const model = {
  ...
  get isGuestUser() {
    return String(this.username || "").startsWith("guest_");
  },

  get shouldRenderShell() {
    return this.isShellVisible && !this.isGuestUser;
  },
  ...
  async init() {
    ...
    this.initializationPromise = (async () => {
      ...
      try {
        const selfInfo = await this.runtime.api?.userSelfInfo?.();
        this.username = String(selfInfo?.username || "").trim();
      } catch {
        this.username = "";
      }
      ...
      try {
        ...
        this.isShellVisible = !this.isGuestUser;
